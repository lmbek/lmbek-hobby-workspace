...
func isVersionOlder(current, required string) bool {
	...
	for i := 0; i < len(rParts) && i < len(cParts); i++ {
		cClean := strings.TrimRight(cParts[i], "abcdefghijklmnopqrstuvwxyz-ABCDEFGHIJKLMNOPQRSTUVWXYZ_+")
		rClean := strings.TrimRight(rParts[i], "abcdefghijklmnopqrstuvwxyz-ABCDEFGHIJKLMNOPQRSTUVWXYZ_+")
		cNum, errC := strconv.Atoi(cClean)
		rNum, errR := strconv.Atoi(rClean)
...
func checkDocker() {
	...
	// Check modern "docker compose" plugin first
