...
func Run() error {
			...
			if !comp.IsGit() {
				...
				if err == nil && fi.IsDir() {
					...
					if gitutil.IsCloned(targetPath) {
						...
						ui.Info("  %-30s  %s (local git)", displayName, strings.Join(parts, "  "))
					} else {
						ui.Info("  %-30s  %s", displayName, ui.ColorGreen+"local directory"+ui.ColorReset)
					}
				} else {
					ui.Warn("  %-30s  %s", displayName, "directory missing (run 'make clone')")
				}
				continue
			}
	...
	fmt.Println()
	ui.Info("Total: %d repos | Cloned/Active: %d | Dirty: %d | Ahead: %d | Behind: %d",
	...
	if behind > 0 {
		ui.Warn("Some repositories are behind remote. Run 'make sync' to update.")
	}
	if cloned < totalRepos {
		ui.Warn("Some repositories/directories are not setup. Run 'make clone' to set up.")
	}
