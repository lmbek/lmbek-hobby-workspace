...
// Sync fetches all remotes and performs a read-only fast-forward pull in the given repository directory.
func Sync(repoDir string) error {
	slog.Debug("Syncing repository (readonly pull)", "dir", repoDir)
	if err := RunGitInDir(repoDir, "fetch", "--all"); err != nil {
		return EnhanceGitError(err)
	}
	return EnhanceGitError(RunGitInDir(repoDir, "pull", "--ff-only"))
}
...
func validateRemoteURL(repo string) error {
	if strings.HasPrefix(repo, "http://") || strings.HasPrefix(repo, "https://") {
		return fmt.Errorf("insecure repository URL detected: %s. This project strictly enforces SSH for Git operations. Please update your repo-definition.yaml to use SSH URLs (e.g., git@github.com:...) or local paths", repo)
	}
	return nil
}
