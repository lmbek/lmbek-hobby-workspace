...
func Run() {
	slog.Info("Starting SSH Setup Tool...")

	fmt.Println("\nSSH Setup Options:")
	fmt.Println("1. Generate a new SSH key")
	fmt.Println("2. Add an existing key to the agent")
	fmt.Println("3. Configure ~/.ssh/config for GitHub")
	if runtime.GOOS == "windows" {
		fmt.Println("4. Configure git to use Windows OpenSSH (Windows only)")
	}
	fmt.Println("5. Check current SSH status")
	fmt.Println("6. Cleanup broken SSH configurations")
	...
	case "4":
		if runtime.GOOS == "windows" {
			...
			slog.Error("Invalid option selected")
		}
	case "5":
		checkStatus(reader)
	case "6":
...
func checkStatus(reader *bufio.Reader) {
	...
	if success {
		...
		if strings.Contains(output, "Permission denied") {
			...
			slog.Info("Try Option 2 (Add existing key to agent) if you are prompted for a passphrase during 'make init'.")
