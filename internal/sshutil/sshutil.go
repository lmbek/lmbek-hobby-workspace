...
func StartAgent() error {
	if runtime.GOOS == "windows" {
		slog.Info("Ensuring ssh-agent service is running on Windows...")
	...
	if runtime.GOOS == "linux" {
		...
		if len(matches) > 0 {
			...
			slog.Info("Found existing ssh-agent socket", "path", matches[0])
			return nil
		}
	}
	...
	for _, line := range lines {
		...
		if strings.HasPrefix(line, "SSH_AUTH_SOCK=") {
			...
			if len(parts) == 2 {
				...
				slog.Info("ssh-agent started and environment set", "SSH_AUTH_SOCK", val)
			}
		}
...
func checkSSH(host string, nonInteractive bool) (bool, string) {
	...
	if !nonInteractive {
		slog.Info("SSH check failed, retrying with verbose output for diagnostics...", "command", sshCmd)
